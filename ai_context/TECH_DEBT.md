...
### 12. ~~Luồng xét điều kiện chưa phân biệt MUA / THUÊ MUA / THUÊ~~ — ĐÃ XỬ LÝ (2026-07-19)
...
**Đã sửa:** thêm `intendedForm: "mua" | "thue_mua" | "thue" | null`. Nêu rõ THUÊ → bỏ qua toàn bộ điều kiện thu nhập/nhà ở, trả `eligible_rent_exempt` kèm trích dẫn Điều 78 **và** Điều 76, headline nói đúng phạm vi ("KHÔNG BỊ RÀNG BUỘC ĐIỀU KIỆN THU NHẬP VÀ NHÀ Ở"), ẩn checklist mua/thuê mua (người thuê không cần giấy xác nhận thu nhập). Chưa nêu hình thức mà bị loại → **code** (không phải LLM) ghép thêm câu nhắc rằng nếu chỉ THUÊ thì điều kiện này không áp dụng.

⚠️ **"Thuê mua" KHÔNG phải "thuê"** — hai hình thức khác nhau về pháp lý, thuê mua vẫn phải đáp ứng đủ điều kiện. Prompt trích xuất đã dặn rõ; có test riêng.
